import { vertex } from './vertex/index';
import fragment from './fragments/colorOverlay.frag';
import { Filter, Color as C} from 'pixi.js';

function hex2rgb(hex: number, out: Array<number> | Float32Array = []) {
    return C.shared.setValue(hex).toRgbArray(out);
}

function rgb2hex(rgb: number[] | Float32Array){
    return C.shared.setValue(rgb).toNumber();
}
